...
public class InAnotherShapeTeller {
    ...
    public boolean isShapeOnTopOfAnotherShape(){
        ...
        for(int i = 1; i < toBeCheckedAgainst.size(); i++){
            ...
            if(((shapeMaxX >= checkingShapeMinX) && (shapeMaxX <= checkingShapeMaxX)) || (checkingShapeMaxX <= shapeMaxX) && (checkingShapeMaxX >= shapeMinX)){
                if(((shapeMaxY >= checkingShapeMinY) && (shapeMaxY <= checkingShapeMaxY)) || ((checkingShapeMaxY <= shapeMaxY) && (checkingShapeMaxY >= shapeMinY))){
                    if(((shapeMinX >= checkingShapeMinX) && (shapeMinX <= checkingShapeMaxX)) || ((checkingShapeMinX >= shapeMinX ) && (checkingShapeMinX <= shapeMaxX))){
                        if(((shapeMinY >= checkingShapeMinY) && (shapeMinY <= checkingShapeMaxY)) || ((checkingShapeMinY >= shapeMinY) && (checkingShapeMinY <= shapeMaxY))){
                            return true;
                          }
                    }
                }
